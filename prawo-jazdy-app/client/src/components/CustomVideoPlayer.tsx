  ...
  return (
    ...
    >
      ...
      {isInteractive && !isPlaying && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/40 hover:bg-black/50 transition-all duration-300 z-10">
          <div className="w-16 h-16 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/20 flex items-center justify-center text-white scale-100 hover:scale-110 active:scale-95 transition-all duration-200 shadow-xl">
            {isEnded ? (
              <RotateCcw className="w-8 h-8 fill-white/10" />
            ) : (
              <Play className="w-8 h-8 fill-white ml-1" />
            )}
          </div>
          <span className="mt-3 text-xs font-semibold text-white/90 tracking-wide uppercase drop-shadow-md">
            {isEnded
              ? "Odtwórz ponownie"
              : isPlaying
              ? "Wznów odtwarzanie"
          ...
          {mode === "practice" && (
            <span className="mt-1 text-[10px] text-white/60 tracking-wider">
